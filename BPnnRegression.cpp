...
int main()
{
	...
	double y_data[5][2] = { { 0.1,0.2 } ,{ 0.1,0.4 } ,{ 0.2,0.1 },{ 0.1,0.1 },{0.3,0.1 } };

	MatrixXf xm(5,3);   xm << 1, 2, 3, 2, 3, 4, 5, 6, 7, 6, 7, 8, 7, 8, 9;
	MatrixXf ym(5,2);   ym <<  0.1,0.2,0.1,0.4,0.2,0.1,0.1,0.1,0.3,0.1;
	MatrixXf xmm = xm.transpose();
	MatrixXf ymm = ym.transpose();
	Vector3f x, x1, z1 ,phy1;
	Vector2f y, x2, z2, phy2;
	...
	for (int i = 0; i < itr; i++)
	{
		...
		if (i % 100 == 0) 
		{
			...
			for (int j = 0; j < 5; j++)
			{
				y2.col(j)= sigmoid(w2 * sigmoid(w1 *xmm.col(j) + b1) + b2);
			}
	...
	cout << "w1:" << endl << w1 <<endl
		...
		<< " w2:" << endl << w2 << endl 
		<< " b2:" << endl << b2 << endl;
	Vector3f x_test(1, 2, 3);
	VectorXf y_test;
	y_test = sigmoid(w2*sigmoid(w1*x_test + b1) + b2);
	cout << y_test;
}
